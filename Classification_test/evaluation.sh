...
#SBATCH --output=console/mnli_2text_base_prints.out       # File to which STDOUT will be written - make sure this is not on $HOME
#SBATCH --error=console/mnli_2text_base_errors.err        # File to which STDERR will be written - make sure this is not on $HOME
...
# Compute Phase
srun --export=ALL python3 training_script.py  # srun will automatically pickup the configuration defined via `#SBATCH` and `sbatch` command line arguments
